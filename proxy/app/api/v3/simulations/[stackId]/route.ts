...
export async function GET(req: NextRequest, { params }: { params: Promise<{ stackId: string }> }) {
  ...
  const simulation = await getV3Simulation(stackId, req.headers.get(BETA_SESSION_HEADER));
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ stackId: string }> }) {
  ...
  try { return NextResponse.json(await transitionV3Simulation(stackId, req.headers.get(BETA_SESSION_HEADER), body.action), { headers: { "cache-control": "no-store" } }); }
