...
export default function V3DemoPage() {
  ...
  return <main className="shell">
    ...
    {simulation && <section className="result">
      ...
      <div className="card"><h2>6. Payment &amp; lifecycle trace</h2><p className="muted">This is the inspectable sandbox trace. A future MoonPay adapter will add provider transaction IDs and verified webhook events here.</p>{simulation.timeline.slice().reverse().map((event) => <p className="event" key={`${event.at}-${event.state}`}><b>{event.state.replaceAll("_", " ")}</b><br/><span className="muted">{event.detail} · {new Date(event.at).toLocaleTimeString()}</span></p>)}<p className="warning">{simulation.warning}</p></div>
    </section>}
    <style jsx>{`
