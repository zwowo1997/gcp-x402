...
The client preserves one request ID across uncertain outcomes. If `v3_trading_deploy` times out or loses its response, call it exactly once more with the same `quoteToken` and `userApproved: true` to reconcile that request; the server returns the already-reserved result without another payment. Never create a fresh quote/request or invoke a legacy `trading-deploy` fallback. A missing local receipt does not prove that no payment or stack exists: report the outcome as unconfirmed until the same-request reconciliation responds. On success, the MCP opens the dashboard privately. For recovery, call `open_v3_dashboard` with the stack ID; never use `v3_trading_receipts` as an actionable dashboard link and never print dashboard credentials.
