...
For private staging, use the internal rewrite configuration and a preview channel so
the public production dashboard continues pointing at the production control plane:

```bash
firebase hosting:channel:deploy trading-internal \
  --config dashboard/firebase.internal.json
```
