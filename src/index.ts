...
server.registerTool("v3_trading_deploy", { title: "Deploy an approved V3 paper stack", description: "After the user clearly approves the one fresh quote just shown, pay exactly its signed Base Sepolia testnet-USDC amount and provision one idempotent paper-only stack. A simple unambiguous affirmative such as ‘yes’, ‘approve’, ‘looks good’, or ‘go ahead’ is sufficient; never ask the user to repeat a quote ID, duration, or price. Mark userApproved true only after that approval. The signed quote is the sole configuration authority and its authorization cap is never transferred.", inputSchema: { quoteToken: z.string().min(20).describe("The signed quoteToken from the immediately preceding v3_trading_quote result. Keep it internal; do not ask the user to repeat it."), userApproved: z.literal(true).describe("Set true only after a clear affirmative for the one displayed, unexpired quote.") } }, async ({ quoteToken, userApproved }) => ({ content: [{ type: "text", text: JSON.stringify(await deployV3PaperTrading({ quoteToken, userApproved }), null, 2) }] }));
