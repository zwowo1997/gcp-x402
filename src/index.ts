...
server.registerTool("v3_trading_quote", { title: "Create a signed V3 paper-stack quote", description: "Create a ten-minute signed quote for a duration and paper strategy. Call this only after the user chooses the Base Sepolia testnet-USDC route; it cannot authorize payment or provision resources. Show its exact expected charge and cap before asking whether to deploy.", inputSchema: { paymentPath: z.literal("testnet-usdc").describe("The user explicitly chose the Base Sepolia testnet-USDC route, rather than the MoonPay showcase."), durationMinutes: z.union([z.literal(15), z.literal(30), z.literal(60)]), fastEma: z.number().int().min(2).optional(), slowEma: z.number().int().min(3).max(64).optional(), virtualBalanceUsd: z.number().positive().optional(), maxOrderNotionalUsd: z.number().positive().optional(), maxPositionNotionalUsd: z.number().positive().optional(), maxDailyLossUsd: z.number().positive().optional(), slippageBps: z.number().min(0).max(100).optional() } }, async ({ paymentPath, durationMinutes, ...strategy }) => ({ content: [{ type: "text", text: JSON.stringify(await quoteV3PaperTrading({ paymentPath, durationMinutes, strategy }), null, 2) }] }));
server.registerTool("v3_trading_deploy", { title: "Deploy an approved V3 paper stack", description: "After the user clearly approves the one fresh quote, pay exactly its signed Base Sepolia testnet-USDC amount and provision one idempotent paper-only stack. The local MCP process opens its private dashboard itself and never exposes dashboard credentials to the model.", inputSchema: { quoteToken: z.string().min(20).describe("The signed quoteToken from the immediately preceding v3_trading_quote result. Keep it internal."), userApproved: z.literal(true).describe("Set true only after a clear affirmative for the one displayed, unexpired quote.") } }, async ({ quoteToken, userApproved }) => { const deployment = await deployV3PaperTrading({ quoteToken, userApproved }); let dashboard: unknown; try { dashboard = { opened: true, ...openTradingDashboard(deployment.stackId), credentialsExposed: false }; } catch (error) { dashboard = { opened: false, message: (error as Error).message, credentialsExposed: false }; } const { capability: _capability, dashboardUrl: _dashboardUrl, ...safe } = deployment; return { content: [{ type: "text", text: JSON.stringify({ ...safe, dashboard }, null, 2) }] }; });
...
server.registerTool("v3_trading_receipts", { title: "List local V3 deployment receipts", description: "List locally stored deployment receipts without capabilities, beta sessions, or other bearer secrets. Dashboard URL fragments are removed. This never starts a payment or deployment.", inputSchema: {} }, async () => ({ content: [{ type: "text", text: JSON.stringify(listTradingReceipts().filter((receipt) => Boolean(receipt.quoteId)).map(publicTradingReceipt), null, 2) }] }));
server.registerTool("open_v3_dashboard", { title: "Open a private V3 trading dashboard", description: "Open a previously deployed stack's private dashboard from this local MCP process. It reads the private local receipt, never prints credentials to the model, and never pays, deploys, or changes the stack.", inputSchema: { stackId: z.string() } }, async ({ stackId }) => ({ content: [{ type: "text", text: JSON.stringify({ opened: true, ...openTradingDashboard(stackId), credentialsExposed: false }, null, 2) }] }));
