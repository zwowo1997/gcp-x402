...
server.registerTool("v3_trading_deploy", { title: "Deploy an approved V3 paper stack", description: "After the user explicitly approves one displayed signed quote, pay exactly its expected Base Sepolia testnet-USDC amount and provision one idempotent paper-only stack. quoteToken, quoteId, durationMinutes, and approvedExpectedChargeUsd must all be from that same fresh quote. Strategy parameters are intentionally not accepted here: the signed quote is the sole configuration authority. The authorization cap is never transferred.", inputSchema: { quoteToken: z.string().min(20).describe("The signed quoteToken returned by v3_trading_quote."), quoteId: z.string().uuid().describe("The quoteId returned by v3_trading_quote and explicitly approved by the user."), durationMinutes: z.union([z.literal(15), z.literal(30), z.literal(60)]), approvedExpectedChargeUsd: z.number().positive().describe("The exact expectedChargeUsd the user explicitly approved for this quote.") } }, async ({ quoteToken, quoteId, durationMinutes, approvedExpectedChargeUsd }) => ({ content: [{ type: "text", text: JSON.stringify(await deployV3PaperTrading({ quoteToken, quoteId, durationMinutes, approvedExpectedChargeUsd }), null, 2) }] }));
