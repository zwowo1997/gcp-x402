...
import { z } from "zod";
import { estimate, query, listDatasets, walletInfo, walletAddress, provisionCatalog, provisionResource, provisionStatus, provisionDelete, tradingCatalog, deployPaperTrading, tradingStatus, controlPaperTrading, unlockService, simulateV3Deployment, v3Catalog } from "./client.js";
...
server.registerTool("v3_catalog", { title: "Inspect v3 sandbox payment plans", description: "Show duration-aware AP2-derived, x402-v2-upto payment plans. This beta endpoint is simulation-only: it cannot transfer funds or create cloud resources.", inputSchema: {} }, async () => ({ content: [{ type: "text", text: JSON.stringify(await v3Catalog(), null, 2) }] }));
server.registerTool("v3_simulate_deployment", { title: "Simulate a v3 Coinbase/onramp deployment", description: "Create a no-risk preview of a Coinbase sandbox handoff, AP2-derived mandate, and temporary GCP resource plan. It is not a payment authorization and provisions nothing.", inputSchema: { productId: z.enum(["trading.paper.ema", "vm.small", "storage.small"]), durationMinutes: z.union([z.literal(15), z.literal(30), z.literal(60)]) } }, async ({ productId, durationMinutes }) => ({ content: [{ type: "text", text: JSON.stringify(await simulateV3Deployment({ productId, durationMinutes }), null, 2) }] }));
